    ...
    */
    public function searchProfil($profilData): array
    {
        return $this->createQueryBuilder('p')
            ...
            ->orWhere('p.metier = :metier')
            ->setParameter('metier', $profilData->metier)
            ->orWhere('h.id IN (:competence)')
            ->setParameter('competence', $profilData->competences)
            ->orWhere('s.id IN (:capacite)')
            ->setParameter('capacite', $profilData->capacites)
            ->getQuery()
            ->getResult()
        ;
    }
